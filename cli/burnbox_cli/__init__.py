...
from .burnbox import BurnBox
from .mode_settings import ModeSettings
from qrcode import QRCode


class LocalOnlyOnion:
    ...
    def connect(self, *args, **kwargs):
        self.common.log("LocalOnlyOnion", "connect", "local-only; skipping Tor")

    def cleanup(self, *args, **kwargs):
        self.common.log("LocalOnlyOnion", "cleanup", "local-only; nothing to do")

    def get_tor_socks_port(self):
        raise RuntimeError("Tor is disabled in --local-only mode")

    def start_onion_service(self, *args, **kwargs):
        raise RuntimeError("Tor is disabled in --local-only mode")

    def stop_onion_service(self, *args, **kwargs):
        return
...
def main(cwd=None):
    ...
    # BurnBox CLI in OSX needs to change current working directory (#132)
    ...
    if local_only:
        onion = LocalOnlyOnion(common)
    else:
        try:
            onion = Onion(common, use_tmp_dir=True)
        except CannotFindTor:
            print("You must install tor to use BurnBox from the command line")
            if common.platform == "Darwin":
    ...
    try:
        ...
        app = BurnBox(common, onion, local_only, autostop_timer)
        app.choose_port()
    ...
    # Start BurnBox http service in new thread
