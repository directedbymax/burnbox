...
class TorErrorAutomatic(Exception):
    """
    BurnBox is failing to connect and authenticate to the Tor controller,
...
class TorErrorSocketPort(Exception):
    """
    BurnBox can't connect to the Tor controller using the supplied address and port.
    """


class TorErrorSocketFile(Exception):
    """
    BurnBox can't connect to the Tor controller using the supplied socket file.
    """


class TorErrorMissingPassword(Exception):
    """
    BurnBox connected to the Tor controller, but it requires a password.
    """


class TorErrorUnreadableCookieFile(Exception):
    """
    BurnBox connected to the Tor controller, but your user does not have permission
...
class TorErrorAuthError(Exception):
    """
    BurnBox connected to the address and port, but can't authenticate. It's possible
...
class TorErrorProtocolError(Exception):
    """
    This exception is raised if burnbox connects to the Tor controller, but it
...
class BundledTorTimeout(Exception):
    """
    This exception is raised if burnbox is set to use the bundled Tor binary,
...
class PortNotAvailable(Exception):
    """
    There are no available ports for BurnBox to use, which really shouldn't ever happen
    """
...
class Onion(object):
    ...
    creating onion services. BurnBox supports creating onion services by
    ...
    def connect(
        ...
        if self.settings.get("connection_type") == "bundled":
                        ...
                        if self.settings.get("bridges_builtin"):
                            try:
                                ...
                                self.common.log(
                                    "Onion",
                                    "connect",
                                    "Wrote in the built-in bridges from BurnBox settings",
                                )
                            except KeyError:
            ...
            try:
                ...
                print("BurnBox could not connect to Tor:\n{}".format(e.args[0]))
