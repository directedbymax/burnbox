    ...
    def define_routes(self):
        ...
        def upload(ajax=False):
            ...
            if (
                ...
                self.send_webhook_notification(f"{msg} submitted to BurnBox")
            ...
            if request.upload_error:
                ...
                print(
                    f"Could not create BurnBox data folder: {request.receive_mode_dir}"
                )

                msg = "Error uploading, please inform the BurnBox user"
                if ajax:
...
class ReceiveModeFile(object):
    ...
    def __init__(self, request, filename, write_func, close_func):
        self.burnbox_request = request
        self.burnbox_filename = filename
        self.burnbox_write_func = write_func
        self.burnbox_close_func = close_func

        self.filename = os.path.join(self.burnbox_request.receive_mode_dir, filename)
        ...
        try:
            ...
            # BurnBox is running, but if it does make sure to throw an error
    ...
    def write(self, b):
        """
        Custom write method that calls out to burnbox_write_func
        """
        if self.upload_error or (not self.burnbox_request.stop_q.empty()):
            self.close()
            self.burnbox_request.close()
            return

        try:
            bytes_written = self.f.write(b)
            self.burnbox_write_func(self.burnbox_filename, bytes_written)
    ...
    def close(self):
        """
        Custom close method that calls out to burnbox_close_func
        """
        try:
            self.f.close()
        ...
        self.burnbox_close_func(self.burnbox_filename, self.upload_error)
    ...
    def __init__(self, environ, populate_request=True, shallow=False):
        ...
        if self.upload_request:
            ...
            try:
                ...
                print(
                    f"Could not create BurnBox data folder: {self.receive_mode_dir}"
                )
                self.web.common.log(
                    "ReceiveModeRequest",
                    "__init__",
