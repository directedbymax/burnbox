...
class ShareModeWeb(SendBaseModeWeb):
    ...
    def define_routes(self):
        ...
        def index(path):
            """
            Render the template for the burnbox landing page.
            """
        ...
        while not self.web.done:
            ...
            if chunk == b"":
                ...
                try:
                    ...
                    # only output to stdout if running burnbox in CLI mode, or if using Linux (#203, #304)
                    if (
                        not self.web.is_gui
...
class ZipWriter(object):
    ...
    with. If a zip_filename is not passed in, it will use the default burnbox
    filename.
    """
    ...
    def __init__(
        ...
        if zip_filename:
            ...
            self.zip_filename = f"{self.zip_temp_dir.name}/burnbox_{self.common.random_string(4, 6)}.zip"
