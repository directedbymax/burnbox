...
class CannotFindTor(Exception):
    """
    BurnBox can't find a tor binary
    """


class Common:
    """
    The Common object is shared amongst all parts of BurnBox.
    """
    ...
    def __init__(self, verbose=False):
        ...
        # The platform BurnBox is running on
        self.platform = platform.system()
        if self.platform.endswith("BSD") or self.platform == "DragonFly":
            self.platform = "BSD"

        # The current version of BurnBox
    ...
    def display_banner(self):
        ...
        try:
            ...
            print(f"BurnBox v{self.version}")
            print("https://burnbox.hideaway.chat/")
            print()
    ...
    def get_resource_path(self, filename):
        ...
        """
        ref = importlib_resources.files("burnbox_cli.resources") / filename
    ...
    def get_tor_paths(self):
        if self.platform == "Linux":
            ...
            # In Windows, the Tor binaries are in the burnbox package, not the burnbox_cli package
            base_path = self.get_resource_path("tor")
            base_path = base_path.replace("burnbox_cli", "burnbox")
            ...
            if not os.path.exists(tor_path):
                ...
                base_path = os.path.join(os.getcwd(), "burnbox", "resources", "tor")
            ...
            # Let's see if we have tor binaries in the burnbox GUI package
            base_path = self.get_resource_path("tor")
            base_path = base_path.replace("burnbox_cli", "burnbox")
    ...
    def build_data_dir(self):
        """
        Returns the path of the BurnBox data directory.
        """
        if self.platform == "Windows":
            try:
                appdata = os.environ["APPDATA"]
                burnbox_data_dir = f"{appdata}\\BurnBox"
            except Exception:
                # If for some reason we don't have the 'APPDATA' environment variable
                # (like running tests in Linux while pretending to be in Windows)
                try:
                    xdg_config_home = os.environ["XDG_CONFIG_HOME"]
                    burnbox_data_dir = f"{xdg_config_home}/burnbox"
                except Exception:
                    burnbox_data_dir = os.path.expanduser("~/.config/burnbox")
        elif self.platform == "Darwin":
            burnbox_data_dir = os.path.expanduser(
                "~/Library/Application Support/BurnBox"
            )
        else:
            try:
                xdg_config_home = os.environ["XDG_CONFIG_HOME"]
        ...
        if getattr(sys, "burnbox_test_mode", False):
            burnbox_data_dir += "-testdata"

        os.makedirs(burnbox_data_dir, 0o700, True)
        return burnbox_data_dir
    ...
    def is_flatpak(self):
        """
        Returns True if BurnBox is running in a Flatpak sandbox
        """
        return os.environ.get("FLATPAK_ID") == "chat.hideaway.burnbox"

    def is_snapcraft(self):
        """
        Returns True if BurnBox is running in a Snapcraft sandbox
        """
        return os.environ.get("SNAP_INSTANCE_NAME") == "burnbox"
    ...
    @staticmethod
    def get_available_port(min_port, max_port, max_attempts=1000):
        """
