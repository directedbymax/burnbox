...
When BurnBox starts, it will show you a screen asking you to connect to the Tor network.

.. image:: _static/screenshots/autoconnect-welcome-screen.png

You can toggle on the switch "Connect to Tor automatically" before clicking "Connect to Tor". This means that next time BurnBox starts, it will automatically connect with its Tor connection settings from the last session, instead of presenting you with the connection options.
...
When you click "Connect to Tor", if BurnBox fails to connect, it might be because Tor is censored in your country or on your local network.
...
If you choose the "Try again without a bridge" option, BurnBox will retry connecting to Tor like normal, without attempting to bypass censorship.
...
BurnBox will temporarily use the `Meek <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/meek/>`_ domain-fronting proxy to make a non-Tor connection from your computer to Tor's Censorship Circumvention API.
...
If the Censorship Circumvention API finds bridges that it believes will suit you, BurnBox will try to reconnect to Tor using those bridges. If the API does not find any bridges for your location, BurnBox will ask the API for "fallback" options, and then try to reconnect using those.

If for some reason BurnBox fails to connect to the Censorship API itself, or if the API returns an error message, BurnBox will attempt to use the obfs4 built-in bridges.
...
Here are the different ways you can configure BurnBox to connect to Tor:

Use the Tor version built into BurnBox
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

This is the default, simplest and most reliable way that BurnBox connects to Tor.
For this reason, it's recommended for most users.

When you open BurnBox, it launches an already configured ``tor`` process in the background for BurnBox to use.
...
To use a bridge, you must select "Use the Tor version built into BurnBox" and check the "Use a bridge" checkbox.
...
Keep in mind you need to keep Tor Browser open in the background while you're using BurnBox for this to work.
...
Open BurnBox, click the "⚙" icon in it, and switch to the Tor Settings tab.
Under "How should BurnBox connect to Tor?" choose "Connect using control port", and set
...
Now configure Tor to allow connections from BurnBox::
...
Open BurnBox, click the "⚙" icon in it, and switch to the Tor Settings tab.
Under "How should BurnBox connect to Tor?" choose "Connect using socket file", and
...
Next, add your user to the group that runs the ``tor`` process (in the case of Debian and Ubuntu, ``debian-tor``) and configure BurnBox to connect to your system ``tor``'s control socket file.
...
After it boots up again, open BurnBox, click the "⚙" icon in it, and switch to the Tor Settings tab.
Under "How should BurnBox connect to Tor?" choose "Connect using socket file".
