...
class UpdateChecker(QtCore.QObject):
    """
    Load http://updates.burnbox.hideaway.chat/latest-version.txt to see what the
    latest version of BurnBox is. If the latest version is newer than the
    ...
    def check(self, force=False):
        ...
        if check_for_updates:
            ...
            try:
                # User agent string includes BurnBox version and platform
                user_agent = f"BurnBox {self.common.version}, {self.common.platform}"
                ...
                update_host = "updates.burnbox.hideaway.chat"

                self.common.log(
                    "UpdateChecker", "check", f"loading http://{update_host}{path}"
                )
                ...
                s.connect((update_host, 80))

                http_request = f"GET {path} HTTP/1.0\r\n"
                http_request += f"Host: {update_host}\r\n"
                ...
                self.common.log(
                    "UpdateChecker",
                    "check",
                    f"latest BurnBox version: {latest_version}",
                )
            ...
            update_url = "https://burnbox.hideaway.chat"
