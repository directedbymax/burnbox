...
from burnbox_cli.censorship import (
    CensorshipCircumvention,
    CensorshipCircumventionError,
)
from burnbox_cli.meek import (
    MeekNotRunning,
    MeekNotFound,
)
...
class AutoConnectTab(QtWidgets.QWidget):
    ...
    def __init__(self, common, tab_id, status_bar, window, parent=None):
        ...
        # Hideaway-style connect screen: no rocket animation
        self.anim_stars = None
        self.anim_ship = None
        self.anim_smoke = None

        # Brand header (flame icon + text)
        self.brand_icon = QtWidgets.QLabel()
        flame_size = 72
        self.brand_icon.setPixmap(
            GuiCommon.svg_to_pixmap(
                GuiCommon.get_resource_path(os.path.join("images", "burnbox_flame.svg")),
                flame_size,
            )
        )
        self.brand_text = QtWidgets.QLabel("BurnBox")
        brand_font = QtGui.QFont()
        brand_font.setPointSize(48)
        brand_font.setWeight(QtGui.QFont.Bold)
        self.brand_text.setFont(brand_font)
        self.brand_text.setStyleSheet("color: #f2f2f2; letter-spacing: 1.2px;")
        ...
        self.image.setLayout(brand_layout)
        ...
        content_widget.setStyleSheet(
            "QWidget { background-color: #0b0b0b; border: 0; border-radius: 18px; }"
        )
        ...
        self.setStyleSheet("QWidget { background-color: #0a0a0a; color: #f2f2f2; }")
    ...
    def check_for_updates(self):
        """
        Check for BurnBox updates in a new thread, if enabled.
        """
...
class AutoConnectFirstLaunchWidget(QtWidgets.QWidget):
    """
    When you first launch BurnBox, this is the widget that is displayed
    """
