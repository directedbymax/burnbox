...
class Application(QtWidgets.QApplication):
    ...
    def __init__(self, common):
        if common.platform == "Linux" or common.platform == "BSD":
            self.setDesktopFileName("chat.hideaway.burnbox")
            self.setOrganizationDomain("burnbox.hideaway.chat")
            self.setOrganizationName("BurnBox")
    ...
    def setDarkMode(self):
        ...
        dark_palette.setColor(QPalette.AlternateBase, QColor(14, 14, 14))
        ...
        dark_palette.setColor(QPalette.Link, QColor(255, 177, 0))
        dark_palette.setColor(QPalette.Highlight, QColor(255, 177, 0))
        ...
        self.setStyleSheet(
            ...
            QWidget {
                ...
                color: #f2f2f2;
                font-family: "Geist", "SF Pro Display", "SF Pro Text", "Avenir Next", "Segoe UI", sans-serif;
                font-size: 13px;
            }
            QFrame { border: 0; background: transparent; }
            QMainWindow { background-color: #000000; }
            QGroupBox { border: 0; margin-top: 16px; background: transparent; }
            QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; padding: 0 0 6px 0; color: #bdbdbd; font-weight: 600; }
            QTabWidget::pane { border: 0; background: #000000; }
            QTabBar {
                background: #0b0b0b;
                border: 0;
            }
...
def main():
    """
    The main() function implements all of the logic that the GUI version of burnbox uses.
    """
    common = Common()
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="verbose",
        help="Log BurnBox errors to stdout, and web errors to disk",
    )
    parser.add_argument(
        "--filenames",
        metavar="filenames",
    ...
    # Is there another burnbox-gui running?
            ...
            for proc in psutil.process_iter(["pid", "name", "username"]):
                if proc.pid == existing_pid:
                    if (
                        ...
                        or "burnbox" not in " ".join(proc.cmdline()).lower()
                    ):
                        still_running = False

        if still_running:
            print(f"Opening tab in existing BurnBox window (pid {existing_pid})")

            # Make an event for the existing BurnBox window
            if filenames:
