...
class ReceiveMode(Mode):
    ...
    def data_dir_button_clicked(self):
        """
        Browse for a new BurnBox data directory, and save to tab settings
        """
        ...
        if selected_dir:
            # If we're running inside a flatpak package, the data dir must be inside ~/BurnBox
            if self.common.gui.is_flatpak:
                if not selected_dir.startswith(os.path.expanduser("~/BurnBox")):
